...
class LoadingScreen:
    ...
    def __init__(self, screen:pg.Surface):
        ...
        self.john_text_rect = self.john_text.get_rect(center=(self.screen.get_width()//2, self.screen.get_height()//2)) # place it in the center

        # "PRESS 'key' TO CONTINUE"
        self.delay_blinking = 0  # delay to animate
        self.decreasing = False  # decreasing alpha or not
        self.text_key = self.font.render(f"Press {pg.key.name(self.get_key())} to continue", True, (255, 255, 255))  # rendering
        self.text_key_rect = self.text_key.get_rect(center=(self.screen.get_width()//2, self.screen.get_height()//2))  # placing
    ...
    def update(self):
        ...
        return {dictionnary} significate the end of the loading screen,
        """
        ...
        if self.main_loading:
            ...
            pg.draw.rect(self.john_text, (0, 0, 0, 0), [(self.john_text_rect.width)/(self.duration)*(pg.time.get_ticks()-self.start_loading),0,self.john_text_rect.width,self.john_text_rect.height])
